...
static Bool running = True, nextfocus, doinitspawn = True,
  fillagain = False, closelastclient = False, killclientsfirst = False;
static Display *dpy;
static DC dc;
...
clientmessage (const XEvent * e) {
  ...
  if (ev->message_type == wmatom[WMProtocols] && ev->data.l[0] == wmatom[WMDelete]) {
...
drawtext (const char *text, XftColor col[ColLast]) {
  ...
  d =
    XftDrawCreate (dpy, dc.drawable, DefaultVisual (dpy, screen),
...
getcolor (const char *colstr) {
  ...
  if (!XftColorAllocName
      (dpy, DefaultVisual (dpy, screen), DefaultColormap (dpy, screen), colstr, &color))
      ...
      if ((code = XKeysymToKeycode (dpy, keys[i].keysym))) {
        for (j = 0; j < LENGTH (modifiers); j++) {
          XGrabKey (dpy, code, keys[i].mod |
                    modifiers[j], w, True, GrabModeAsync, GrabModeAsync);
        }
      }
    }
  ...
  if (ev->state == PropertyNewValue && ev->atom == wmatom[WMSelectTab]) {
    ...
    if (!strncmp (selection, "0x", 2)) {
      ...
      spawn (&arg);
             ...
             (c = getclient (ev->window)) > -1 && (wmh = XGetWMHints (dpy, clients[c]->win))) {
...
setup (void) {
  ...
  XSelectInput (dpy, win, SubstructureNotifyMask | FocusChangeMask |
                ...
                PropertyChangeMask | StructureNotifyMask | SubstructureRedirectMask);
...
xsettitle (Window w, const char *str) {
  ...
  if (XmbTextListToTextProperty (dpy, (char **) &str, 1, XCompoundTextStyle, &xtp) == Success) {
...
case 'u':
    ...
    break;
case 'v':
    die ("tabbed-" VERSION ", © 2009-2016 tabbed engineers, " "see LICENSE for details.\n");
    break;
default:
    usage ();
    break;
